...
import HodViewDetail from "./AdminHodDetail";
import AdminCreateHod from "./AdminCreateHod";
import AdminUpdateHod from "./AdminUpdateHod"; 
...
export default function AdminHods() {
  ...
  // state variables inside the AdminHods component
  const [viewModalOpen, setViewModalOpen] = useState(false);
  const [selectedHodId, setSelectedHodId] = useState<string | null>(
    null
  );
  const [createModalOpen, setCreateModalOpen] = useState(false);
  const [updateModalOpen, setUpdateModalOpen] = useState(false); 
  const [editingHodId, setEditingHodId] = useState<string | null>(null); 

  // Add handlers for opening/closing the update modal
  const handleEditHod = (hodId: string) => {
    setEditingHodId(hodId);
  ...
  const handleViewHod = (hodId: string) => {
  ...
  const handleDeleteHod = async (
    ...
    const message = `Are you sure you want to delete this hod?\n\n${name_kh} (${name_en})`;
  ...
  const handleSort = (column: "hod_id" | "name_en" | "name_kh" | "dob") => {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
     ...
     text-base placeholder-gray-500 transition-colors"
          ...
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th
                ...
                onClick={() => handleSort("hod_id")}
              >
                <div className="flex items-center gap-1">
                  ID
                  <span className="ml-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {filters.sort_by === "hod_id" &&
      ...
      <AdminCreateHod
        ...
        onSuccess={() => {
          // Refresh the hod list after successful creation
          fetchHods();
      ...
      {editingHodId && (
        <AdminUpdateHod
          ...
          onSuccess={() => {
            // Refresh the hod list after successful update
            fetchHods();
          }}
        />
      )}
