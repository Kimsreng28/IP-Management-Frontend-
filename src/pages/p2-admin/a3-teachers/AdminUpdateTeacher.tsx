...
export default function AdminUpdateTeacher({
  ...
  // Get current teacher data when modal opens
  useEffect(() => {
    if (isOpen && teacherId) {
      fetchTeacherData();
    }
  ...
  const fetchTeacherData = async () => {
    ...
    try {
      ...
      if (teacher) {
        ...
        if (teacher.image) {
          setOriginalImage(teacher.image);
          ...
          const firstLetter = teacher.name_en?.[0]?.toUpperCase() || "?";
          setImagePreview(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.image) {
        ...
        await updateTeacher(teacherId, formDataToSend as any);
      } else {
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="border-t border-gray-200 px-6 py-4 bg-gray-50 flex justify-end gap-3">
          <button
            ...
            disabled={isLoading || isFetchingTeacher}
          >
            Cancel
          </button>
          <button
